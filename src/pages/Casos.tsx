...
export default function Casos() {
  ...
  const { data: journalAll, refetch: refetchJournal } = useApiData<any[]>({ path: "/api/client-journal", params: selectedId ? { case_id: selectedId } : undefined, mockData: mockJournalEntries, enabled: !!selectedId });
        ...
        {tab === "journal" && (
          <div className="space-y-3">
            ...
            <div className="bg-card rounded-lg border p-4 space-y-2">
              <textarea
                placeholder="Nueva entrada de journal..."
                rows={2}
                value={journalText}
                onChange={(e) => setJournalText(e.target.value)}
                className="w-full px-3 py-2 rounded-lg border bg-background text-sm outline-none resize-none focus:ring-2 focus:ring-teal/50"
              />
              <div className="flex items-center gap-2">
                <select
                  value={journalType}
                  onChange={(e) => setJournalType(e.target.value)}
                  className="px-2 py-1.5 rounded border bg-background text-xs outline-none"
                >
                ...
                <button
                  onClick={async () => {
                    if (!journalText.trim() || !selected) return;
                    try {
                  ...
                  className="px-3 py-1.5 rounded bg-teal text-accent-foreground text-xs font-medium hover:bg-teal/90 transition-colors"
                >
                  Agregar
                </button>
              </div>
